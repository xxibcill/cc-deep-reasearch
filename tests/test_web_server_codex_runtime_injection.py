...
def test_route_level_content_agents_use_injected_runtime(
    ...
    agent = agent_factory(Config(), codex_runtime=runtime)
...
def test_pipeline_stage_agents_use_injected_runtime(
    ...
    runtime = object()
    pipeline = ContentGenPipeline(Config(), codex_runtime=runtime)
...
def test_content_service_graph_owns_one_codex_runtime() -> None:
    ...
    assert services.codex_runtime is runtime
    assert services.pipeline_service._codex_runtime is runtime
    assert services.scripting_api_service._codex_runtime is runtime
