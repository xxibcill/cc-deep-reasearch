...
export async function getStrategyReadiness(): Promise<import('@/types/content-gen').StrategyReadinessResult> {
  try {
    const response = await contentGenClient.get('/strategy/readiness');
    return response.data;
  } catch (error) {
    throw new Error(getApiErrorMessage(error, 'Failed to get strategy readiness.'));
  }
}

export async function getRulesForReview(): Promise<import('@/types/content-gen').RuleVersion[]> {
  try {
    const response = await contentGenClient.get('/strategy/rules-for-review');
    return response.data.items ?? response.data;
  } catch (error) {
    throw new Error(getApiErrorMessage(error, 'Failed to get rules for review.'));
  }
}
...
): Promise<import('@/types/content-gen').RuleVersion> {
  try {
    ...
    throw new Error(getApiErrorMessage(error, 'Failed to update rule lifecycle.'));
  }
}

// ---------------------------------------------------------------------------
// P4-T3: Operating Fitness endpoints
// ---------------------------------------------------------------------------
...
export async function getOperatingFitness(
  ...
  try {
    ...
    throw new Error(getApiErrorMessage(error, 'Failed to get operating fitness.'));
  }
}
